...
```text
cmd/
  publisher/       Worker poll campaign pending và publish vào SQS
...
1. Chạy `publisher` như worker dài hạn để poll campaign pending theo `PUBLISHER_POLL_SECONDS`.
2. `internal/db.PendingCampaigns` lấy tối đa `PUBLISHER_CLAIM_LIMIT` campaign `pending`, rồi `MarkCampaignProcessing` update từng campaign sang `processing`.
...
Publisher poll campaign pending liên tục và xử lý nhiều campaign song song:

```text
PUBLISHER_POLL_SECONDS=5
PUBLISHER_WORKERS=4
PUBLISHER_CLAIM_LIMIT=10
```
...
```powershell
docker compose --profile app up subscriber line-mock publisher
```

Nếu muốn chạy publisher ở terminal riêng để dễ nhìn log:
...
Start line mock, subscriber và publisher:

```powershell
docker compose --profile app up subscriber line-mock publisher
```

Hoặc chạy publisher riêng:
...
Nên test từng case riêng. Publisher hiện là worker polling, nên hãy để publisher chạy trước, sau đó insert campaign `pending`; publisher sẽ tự pick campaign ở tick tiếp theo.

```powershell
docker compose up -d mysql redis localstack
docker compose --profile app up -d line-mock subscriber publisher
```

Mở terminal khác để xem log trong lúc test:

```powershell
docker compose logs -f publisher subscriber line-mock
```
...
```powershell
docker compose exec mysql mysql -utraining -ptraining training_msg_queue -e "INSERT INTO message_campaigns (name, segment_file_key, status) VALUES ('Happy path test', 'segments/active-users.json', 'pending');"
```

Chờ vài giây để publisher poll và subscriber consume message.

Kiểm tra DB:

```powershell
docker compose exec mysql mysql -utraining -ptraining training_msg_queue -e "SELECT id, name, status FROM message_campaigns ORDER BY id DESC LIMIT 3; SELECT id, user_id, line_user_id, status, sqs_message_id FROM outbox_messages ORDER BY id DESC LIMIT 5;"
```
...
```powershell
docker compose exec mysql mysql -utraining -ptraining training_msg_queue -e "SELECT id, campaign_id, user_id, line_user_id, idempotency_key FROM outbox_messages WHERE status = 'consumed' LIMIT 1;"
```

Gửi duplicate vào SQS:

Nếu query ở bước trên trả về id/key khác, thay `campaign_id`, `outbox_id`, `user_id`, `line_user_id`, và `idempotency_key` trong JSON dưới đây cho khớp.
...
```powershell
docker compose exec mysql mysql -utraining -ptraining training_msg_queue -e "INSERT INTO message_campaigns (name, segment_file_key, status) VALUES ('Retry DLQ test', 'segments/active-users.json', 'pending');"
```
...
```powershell
docker compose exec mysql mysql -utraining -ptraining training_msg_queue -e "INSERT INTO message_campaigns (name, segment_file_key, status) VALUES ('Missing S3 test', 'segments/not-found.json', 'pending');"
```

Chờ vài giây để publisher poll campaign.

Kiểm tra DB:

```powershell
docker compose exec mysql mysql -utraining -ptraining training_msg_queue -e "SELECT id, name, status, error_message FROM message_campaigns ORDER BY id DESC LIMIT 1;"
```
...
```powershell
docker compose exec mysql mysql -utraining -ptraining training_msg_queue -e "INSERT INTO message_campaigns (name, segment_file_key, status) VALUES ('Redis down fallback test', 'segments/active-users.json', 'pending');"
```
...
```powershell
docker compose exec mysql mysql -utraining -ptraining training_msg_queue -e "DELETE FROM outbox_messages; DELETE FROM message_campaigns; INSERT INTO message_campaigns (name, segment_file_key, status) VALUES ('Training campaign 001', 'segments/active-users.json', 'pending');"
```
...
docker compose down
docker compose up -d mysql redis localstack
docker compose --profile app up --build -d subscriber line-mock publisher
```
